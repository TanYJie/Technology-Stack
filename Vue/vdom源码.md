...
export default class VNode {
  tag: string | void;      // 标签名
  data: VNodeData | void;  // 数据对象，具体包含哪些字段可以参考vue源码types/vnode.d.ts中对VNodeData的定义
...
return function patch (oldVnode, vnode, hydrating, removeOnly) {
    ...
    // vnode不存在但是oldVnode存在，要销毁老节点，那么就调用invokeDestroyHook(oldVnode)来进行销毁
    if (isUndef(vnode)) {
    ...
    if (isUndef(oldVnode)) {
      ...
      // oldVnode不存在但是vnode存在，说明意图是要创建新节点，那么就调用createElm来创建新节点
      isInitialPatch = true
      ...
      if (!isRealElement && sameVnode(oldVnode, vnode)) {
        // 当oldVnode和vnode是同一个节点时
        // 就调用patchVnode来进行patch
        ...
        if (isRealElement) {
          ...
          if (isTrue(hydrating)) {
            //如果oldVnode是真实dom节点或hydrating设置为true，需要用hydrate函数将虚拟dom和真实dom进行映射
